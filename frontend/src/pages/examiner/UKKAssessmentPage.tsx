...
export const UKKAssessmentPage = () => {
  ...
  const activeAcademicYear = useMemo(() => {
    const years = academicYearData?.data?.academicYears || academicYearData?.academicYears || [];
    return years.find((ay: any) => ay.isActive) || years[0];
  }, [academicYearData]);
  ...
  const { data: schemesData, isLoading: isLoadingSchemes } = useQuery({
    ...
    enabled: !schemeId && !!activeAcademicYear?.id
  });
  ...
  const { data: schemeData, isLoading: isLoadingScheme } = useQuery({
    queryKey: ['ukk-scheme', schemeId],
    queryFn: () => ukkSchemeService.getSchemeDetail(Number(schemeId)),
    enabled: !!schemeId
  });

  const scheme = schemeData?.data || schemeData;
  ...
  const { filteredStudents, availableClasses } = useMemo(() => {
    if (!studentsData?.data) return { filteredStudents: [], availableClasses: [] };
    
    // 1. Base Filter (Major & Level)
    const baseStudents = studentsData.data.filter((student: any) => {
    ...
    baseStudents.forEach((s: any) => {
      if (s.studentClass) {
        classesMap.set(s.studentClass.id, s.studentClass);
      }
    });
    const classes = Array.from(classesMap.values()).sort((a: any, b: any) => a.name.localeCompare(b.name));

    // 3. Apply Search & Class Filter
    const finalStudents = baseStudents.filter((student: any) => {
    ...
    finalStudents.sort((a: any, b: any) => {
    ...
    if (existingAssessments.length > 0 && scheme && scheme.criteria) {
      ...
      scheme.criteria.forEach((c: any) => {
        if (c.id) {
           ...
           nameToIdMap.set(key, c.id);
           if (Array.isArray(c.aliases)) {
             c.aliases.forEach((aliasKey: string) => {
               aliasToIdMap.set(aliasKey, c.id);
             });
           }
        }
      });
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => ukkAssessmentService.upsertAssessment(data),
    onSuccess: () => {
  ...
  const handleScoreChange = (studentId: number, criterion: any, value: string) => {
  ...
  const handleSaveAll = async () => {
    ...
    for (const studentId of studentIds) {
      ...
      const maxTotal = scheme.criteria.reduce((a: number, b: any) => a + Number(b.maxScore), 0);
  ...
  const handleExportCSV = () => {
    ...
    const criteriaNames = Object.values(groupedCriteria).flat().map((c: any) => c.name);
    headers.push(...criteriaNames, 'Total Skor', 'Nilai Akhir (0-100)');
    
    // Rows
    const rows = filteredStudents.map((student: any, index: number) => {
        const studentScores = allScores[student.id] || {};
        const scores = Object.values(groupedCriteria).flat().map((c: any) => {
            const key = getCriteriaKey(c);
                        ...
                        >
                            ...
                            <div className="space-y-1.5">
                                <p className="text-sm text-gray-600 flex items-center gap-2">
                                    <span className="font-medium text-gray-900">Jurusan:</span> {scheme.major?.name || '-'}
                                </p>
                            </div>
                        </Link>
                    ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] border-collapse">
            <thead className="bg-gray-50 border-b border-gray-200">
              ...
              <tr>
                {Object.values(groupedCriteria).flat().map((criterion: any, idx: number) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredStudents.length === 0 ? (
                <tr>
                  <td colSpan={3 + (scheme?.criteria.length || 0)} className="px-6 py-12 text-center text-gray-500 text-sm">
                ...
                filteredStudents.map((student: any, index: number) => {
                  ...
                  const totalScore = scheme.criteria.reduce((acc: number, c: any) => {
                      const key = getCriteriaKey(c);
