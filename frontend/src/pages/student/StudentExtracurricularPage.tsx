...
export const StudentExtracurricularPage = () => {
  ...
  const { data: listRes, isLoading: loadingList } = useQuery<ListResponse>({
    queryKey: ['public-extracurriculars', search],
    queryFn: async () => {
  ...
  const items = useMemo(() => listRes?.extracurriculars ?? [], [listRes?.extracurriculars]);
  ...
  const availableItems = useMemo(
    ...
    [canChooseRegularExtracurricular, canRequestOsis, items],
  );
  const hasSearchTerm = search.trim().length > 0;
  const selectedItem = availableItems.find((item) => item.id === selectedAgreeItemId) || null;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Pilih 1 ekstrakurikuler reguler dan, jika tersedia, ajukan OSIS secara terpisah pada tahun ajaran aktif.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-lg border border-yellow-200 bg-yellow-50 p-4 text-yellow-800">
        ...
        <div className="text-sm">
          Anda tetap hanya boleh memilih <span className="font-semibold">1 ekstrakurikuler reguler</span> per tahun
          ajaran. OSIS diperlakukan sebagai organisasi siswa, sehingga pengajuannya tidak mengunci slot ekskul reguler
          dan akan diproses oleh pembina OSIS ke divisi serta jabatan yang sesuai.
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        {availableItems.length > 0 ? (
          ...
          <div className="px-6 py-8 text-sm text-slate-600">
            {hasSearchTerm
              ? 'Tidak ada ekstrakurikuler atau OSIS yang cocok dengan pencarian Anda.'
              : myEnrollment && !canRequestOsis
              ? 'Pilihan ekskul reguler Anda sudah terkunci dan status OSIS Anda juga sudah diproses.'
              : myEnrollment
                ? 'Ekskul reguler Anda sudah terkunci. Jika sekolah menyediakan OSIS, pengajuannya diproses terpisah.'
                : !canChooseRegularExtracurricular
                  ? 'Pilihan ekskul reguler Anda sudah terkunci untuk tahun ajaran aktif.'
