...
export const TeacherHomeroomSatPage = () => {
  const [activeTab, setActiveTab] = useState<TabType>('ledger');
  
  // SAT is typically Semester Genap (EVEN)
  const semester = 'EVEN';
  ...
  useEffect(() => {
    if (userData?.data?.preferences) {
        // @ts-ignore
        const savedTab = userData.data.preferences['teacher-homeroom-sat-active-tab'];
        if (savedTab) {
  ...
  const handleTabChange = (tab: TabType) => {
    ...
    if (userId) {
        ...
        updateProfileMutation.mutate({
            preferences: { ...currentPrefs, 'teacher-homeroom-sat-active-tab': tab }
        });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Rapor SAT</h1>
          <p className="text-gray-500 text-sm">
            Kelola rapor Sumatif Akhir Tahun (Genap), leger nilai, dan ekstrakurikuler
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[500px]">
          ...
          <div className="border-b border-gray-200 bg-gray-50 p-4 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 text-gray-600 border border-gray-200 text-sm font-medium">
              <span>Semester {semester === 'EVEN' ? 'Genap' : 'Ganjil'}</span>
            </div>
          </div>
