...
type ViewTab = 'TEACHER' | 'CLASS' | 'ISSUES';
...
const VIEW_TABS = [
  ...
  { id: 'CLASS', label: 'Per Kelas', icon: BarChart3 },
  { id: 'ISSUES', label: 'Perlu Ditindaklanjuti', icon: AlertTriangle },
];
  ...
  return (
    <div className="min-w-[150px]">
      ...
      <div className="h-2 overflow-hidden rounded-full bg-gray-100">
        <div className="h-2 rounded-full bg-blue-600" style={{ width: `${Math.min(100, Math.max(0, row.complianceRate))}%` }} />
      </div>
...
export default function TeachingJournalMonitoringPage() {
  const [rangeTab, setRangeTab] = useState<RangeTab>('WEEK');
  const [viewTab, setViewTab] = useState<ViewTab>('TEACHER');
  ...
  const monitoringQuery = useQuery({
    queryKey: ['teaching-journal-monitoring', activeAcademicYearId, range.startDate, range.endDate, search],
  ...
  const isLoading = isLoadingActiveYear || monitoringQuery.isLoading;
  ...
  const renderTeacherRows = (rows: TeachingJournalMonitoringTeacherRow[]) => (
    <table className="w-full min-w-[1040px] text-left">
      <thead className="bg-gray-50">
        <tr className="border-b border-gray-200">
          ...
          <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500">Referensi</th>
      ...
      <tbody className="divide-y divide-gray-100">
        {rows.map((row, index) => (
          <tr key={row.teacher.id} className="hover:bg-gray-50">
            ...
            <td className="px-5 py-4 text-sm text-gray-700">
              <p>{row.referenceLinkedSessions} jurnal</p>
              <p className="text-xs text-gray-500">CP {row.referenceFields.competency || 0} • TP {row.referenceFields.learningObjective || 0}</p>
            </td>
            <td className="px-5 py-4">
              <RateCell row={row} />
            </td>
      ...
      <tbody className="divide-y divide-gray-100">
        {rows.map((row, index) => (
          <tr key={row.class.id} className="hover:bg-gray-50">
            ...
            <td className="px-5 py-4 text-sm text-gray-700">
              <p>{row.referenceLinkedSessions} jurnal</p>
              <p className="text-xs text-gray-500">Materi {row.referenceFields.materialScope || 0} • Indikator {row.referenceFields.indicator || 0}</p>
            </td>
            <td className="px-5 py-4">
              <RateCell row={row} />
            </td>
    ...
    <table className="w-full min-w-[1120px] text-left">
      <thead className="bg-gray-50">
        <tr className="border-b border-gray-200">
          ...
          <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500">Catatan</th>
        </tr>
      </thead>
      <tbody className="divide-y divide-gray-100">
        {rows.map((row, index) => (
  ...
  const activeRowsCount = viewTab === 'TEACHER' ? teacherRows.length : viewTab === 'CLASS' ? classRows.length : issueRows.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-5">
        ...
        <SummaryCard label="Belum Diisi" value={`${summary.missingSessions}`} helper={`${summary.draftSessions} masih draft`} tone="border-rose-100 bg-rose-50 text-rose-700" />
        <SummaryCard label="Presensi" value={formatPercent(summary.attendanceRate)} helper={`${summary.attendanceMismatch} mismatch`} tone="border-amber-100 bg-amber-50 text-amber-700" />
      ...
      <section className="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-section-title text-gray-900">Tabel Monitoring</h2>
            <p className="mt-1 text-sm text-gray-500">
              Data mengikuti tahun ajaran aktif dan rentang tanggal yang dipilih.
            </p>
          </div>
          <UnderlineTabBar
            items={VIEW_TABS}
        ...
        {isLoading ? (
          <div className="flex min-h-[320px] flex-col items-center justify-center text-gray-500">
            ...
            <p>Memuat monitoring jurnal mengajar...</p>
          </div>
        ) : monitoringQuery.isError ? (
          <div className="flex min-h-[260px] flex-col items-center justify-center px-5 text-center">
            <p className="text-sm font-semibold text-gray-900">Gagal memuat monitoring jurnal.</p>
          ...
          <div className="overflow-x-auto">
            {viewTab === 'TEACHER' ? renderTeacherRows(teacherRows) : null}
            {viewTab === 'CLASS' ? renderClassRows(classRows) : null}
