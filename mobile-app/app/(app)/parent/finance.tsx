...
export default function ParentFinanceScreen() {
  ...
  const bankAccountsQuery = useQuery({
    queryKey: ['parent-finance-portal-bank-accounts-mobile'],
    enabled: isAuthenticated,
    queryFn: () => parentApi.getPortalBankAccounts(),
  });
  ...
  const handleSubmitPayment = async () => {
    if (!selectedChildFinance || !selectedInvoice) {
      Alert.alert('Belum ada tagihan', 'Belum ada tagihan aktif untuk anak yang dipilih.');
      return;
    }
    if (!proofFile) {
    ...
    try {
      ...
      const uploaded = await parentApi.uploadPaymentProof({
        uri: proofFile.uri,
        name: proofFile.name,
        type: proofFile.mimeType || undefined,
      });
      await parentApi.submitPayment({
                    ...
                    <View style={{ marginTop: 12 }}>
                      ...
                      <View style={{ marginHorizontal: -4, flexDirection: 'row', flexWrap: 'wrap' }}>
                        {outstandingInvoices.map((invoice) => {
                          ...
                          return (
                            <View key={`parent-submit-invoice-${invoice.id}`} style={{ width: '50%', paddingHorizontal: 4, marginBottom: 8 }}>
                              <Pressable
                                onPress={() => {
                      ...
                      <View style={{ marginHorizontal: -4, flexDirection: 'row', flexWrap: 'wrap' }}>
                        {[
                          ...
                          return (
                            <View key={`parent-submit-method-${option.value}`} style={{ width: '33.33%', paddingHorizontal: 4, marginBottom: 8 }}>
                              <Pressable
                                onPress={() => setPaymentMethod(option.value as typeof paymentMethod)}
                                style={{
                                  borderWidth: 1,
                                  borderColor: selected ? BRAND_COLORS.blue : '#d5e1f5',
                                  backgroundColor: selected ? '#e9f1ff' : '#fff',
                      ...
                      <View style={{ marginHorizontal: -4, flexDirection: 'row', flexWrap: 'wrap' }}>
                        {(bankAccountsQuery.data || []).map((account) => {
                          ...
                          return (
                            <View key={`parent-submit-account-${account.id}`} style={{ width: '100%', paddingHorizontal: 4, marginBottom: 8 }}>
                              <Pressable
