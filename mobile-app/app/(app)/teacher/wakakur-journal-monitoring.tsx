...
type ViewTab = 'TEACHERS' | 'CLASSES' | 'ISSUES';
...
const VIEW_ITEMS: Array<{ key: ViewTab; label: string; iconName: React.ComponentProps<typeof Feather>['name'] }> = [
  { key: 'TEACHERS', label: 'Per Guru', iconName: 'users' },
  { key: 'CLASSES', label: 'Per Kelas', iconName: 'grid' },
  { key: 'ISSUES', label: 'Tindak Lanjut', iconName: 'alert-triangle' },
];
  ...
  return (
    <View style={{ borderWidth: 1, borderColor: '#dbe7fb', borderRadius: 14, backgroundColor: '#fff', padding: 12 }}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
        <MetricChip label="Sesi" value={formatNumber(row.expectedSessions)} bodyTextStyle={bodyTextStyle} />
        <MetricChip label="Terkirim" value={formatNumber(row.submittedAndReviewed)} bodyTextStyle={bodyTextStyle} />
        <MetricChip label="Presensi" value={`${formatNumber(row.attendanceRecorded)}/${formatNumber(row.expectedSessions)}`} bodyTextStyle={bodyTextStyle} />
        <MetricChip label="Referensi" value={formatNumber(row.referenceLinkedSessions)} bodyTextStyle={bodyTextStyle} />
      </View>
    </View>
  );
}
        ...
        {row.issueLabels.map((label) => (
          <View
            ...
            style={{
              borderRadius: 999,
              borderWidth: 1,
              borderColor: label.toLowerCase().includes('referensi') ? '#bfdbfe' : '#fecdd3',
              backgroundColor: label.toLowerCase().includes('referensi') ? '#eff6ff' : '#fff1f2',
              paddingHorizontal: 9,
              paddingVertical: 5,
            }}
          >
            <Text
              style={{
                color: label.toLowerCase().includes('referensi') ? BRAND_COLORS.blue : '#be123c',
                fontWeight: '700',
                ...helperTextStyle,
              }}
            >
...
export default function TeacherWakakurJournalMonitoringScreen() {
  ...
  const [viewTab, setViewTab] = useState<ViewTab>('TEACHERS');
  ...
  const activeYearQuery = useQuery({
    queryKey: ['mobile-wakakur-journal-monitoring-active-year'],
  ...
  const monitoringQuery = useQuery({
    queryKey: [
      'mobile-wakakur-journal-monitoring-data',
  ...
  const summary = monitoringQuery.data?.summary || EMPTY_SUMMARY;
  const teacherRows = monitoringQuery.data?.teacherRows || [];
  const classRows = monitoringQuery.data?.classRows || [];
  const issueRows = monitoringQuery.data?.issueRows || [];
  const activeCount = viewTab === 'TEACHERS' ? teacherRows.length : viewTab === 'CLASSES' ? classRows.length : issueRows.length;

  if (isLoading) return <AppLoadingScreen message="Memuat monitoring jurnal..." />;
  ...
  return (
    ...
    >
      ...
      <Text style={{ color: BRAND_COLORS.textMuted, marginBottom: 12, ...inputTextStyle }}>
        Pantau pengisian jurnal, presensi mapel, dan coverage perangkat ajar guru.
      </Text>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', marginBottom: 10 }}>
        <View style={{ width: '48.5%', marginBottom: 8 }}>
          <MobileSummaryCard
            ...
            value={formatNumber(summary.expectedSessions)}
        ...
        <View style={{ width: '48.5%', marginBottom: 8 }}>
          <MobileSummaryCard
            title="Belum Diisi"
            value={formatNumber(summary.missingSessions)}
            subtitle={`${formatNumber(summary.draftSessions)} masih draft`}
            iconName="file-minus"
            accentColor="#ef4444"
          />
        </View>
      ...
      {monitoringQuery.isLoading ? <QueryStateView type="loading" message="Memuat monitoring jurnal..." /> : null}

      {monitoringQuery.isError ? (
        <View style={{ marginBottom: 12 }}>
          <QueryStateView
            type="error"
            message="Gagal memuat monitoring jurnal mengajar."
      ...
      {monitoringQuery.data ? (
        ...
        >
          ...
          {activeCount === 0 ? (
            <QueryStateView type="error" message="Tidak ada data pada filter saat ini." />
          ) : null}
