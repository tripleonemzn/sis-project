...
export default function InternalViewerScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const params = useLocalSearchParams<{ viewerKey?: string; path?: string; url?: string; label?: string }>();
  ...
  const viewerKey = typeof params.viewerKey === 'string' ? params.viewerKey : '';
  ...
  const effectiveLabel = labelOverride || 'Viewer Internal';
  const targetUrl = useMemo(
    () => (urlOverride || (pathOverride ? resolveWebUrl(pathOverride) : null)),
    [pathOverride, urlOverride],
  );
  const embeddedUrl = useMemo(() => (targetUrl ? buildEmbeddedUrl(targetUrl) : null), [targetUrl]);
  const shouldHideWebChrome = useMemo(() => Boolean(targetUrl && isInternalDashboardUrl(targetUrl)), [targetUrl]);
  ...
  if (isLoading) return <AppLoadingScreen message="Menyiapkan viewer..." />;
  ...
  if (accessToken === undefined) {
    return <AppLoadingScreen message="Menyiapkan akses viewer..." />;
  }
    ...
    <View style={{ flex: 1, backgroundColor: '#f8fafc', ...pagePadding }}>
      ...
      >
        {panelError ? (
          ...
          <WebView
            key={`mobile-viewer-${viewerKey}-${webviewKey}`}
            source={{
              uri: embeddedUrl,
          ...
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 20 }}>
            <Text style={{ color: BRAND_COLORS.textMuted, textAlign: 'center' }}>
              URL konten belum tersedia untuk viewer ini.
            </Text>
          </View>
        )}
      </View>
