...
export function resolveProfileEducationTrackForRole(role?: string | null): ProfileEducationTrack {
...
export function resolveEducationSummaryFromHistories(
  ...
  const sanitized = sanitizeEducationHistories(histories, track, options).filter((entry) =>
    hasEducationHistoryContent(entry),
  );
