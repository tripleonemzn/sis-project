...
const INVENTORY_TEMPLATE_PROFILES: Record<InventoryTemplateKey, InventoryTemplateProfile> = {
  ...
  SPORTS: {
    ...
    purchaseInfoLabel: 'Info Pembelian',
    showPurchaseInfo: true,
    attributeFields: [{ key: 'sizeSpec', label: 'Ukuran / Spesifikasi', type: 'text', placeholder: 'Contoh: Size 5', table: true }],
  },
  OFFICE: {
    key: 'OFFICE',
    label: 'Perkantoran',
...
export const INVENTORY_TEMPLATE_OPTIONS = [
  {
  ...
  },
  {
    ...
    label: 'Inventaris Perpustakaan',
    hint: 'Fokus data buku: judul, penulis, penerbit, tahun terbit, dan kategori.',
  },
  {
    key: 'LAB' as const,
    label: 'Laboratorium',
    hint: 'Fokus data alat praktik: serial number dan catatan perawatan.',
  },
  {
    key: 'SPORTS' as const,
    label: 'Olahraga',
    hint: 'Fokus data peralatan olahraga: ukuran atau spesifikasi alat.',
  },
  {
    key: 'OFFICE' as const,
    label: 'Perkantoran',
...
export function guessTemplateKeyFromCategoryName(name?: string | null): InventoryTemplateKey {
  ...
  if (normalized.includes('lab') || normalized.includes('praktik') || normalized.includes('laboratorium')) return 'LAB';
  if (normalized.includes('olahraga') || normalized.includes('sport')) return 'SPORTS';
  if (normalized.includes('kantor') || normalized.includes('office') || normalized.includes('tata usaha')) return 'OFFICE';
  return 'STANDARD';
}
