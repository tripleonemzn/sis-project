...
Usage:
  ...
  bash ./scripts/set-academic-rollover-flag.sh off [--reload]
...
read_flag_value() {
  local env_file="$1"
  local value
  value="$(grep -E '^ACADEMIC_YEAR_ROLLOVER_ENABLED=' "$env_file" | tail -n1 | cut -d'=' -f2- || true)"
  value="${value%\"}"
  value="${value#\"}"
...
if [ "$ACTION" = "check" ]; then
  echo "Academic year rollover flag: $CURRENT_VALUE"
...
if [ "$CURRENT_VALUE" = "$NEXT_VALUE" ]; then
  echo "Academic year rollover flag sudah $NEXT_VALUE."
else
  BACKUP_FILE="${ENV_FILE}.rollover-flag.$(date +%Y%m%d_%H%M%S).bak"
  ...
  if grep -qE '^ACADEMIC_YEAR_ROLLOVER_ENABLED=' "$ENV_FILE"; then
    sed 's/^ACADEMIC_YEAR_ROLLOVER_ENABLED=.*/ACADEMIC_YEAR_ROLLOVER_ENABLED='"$NEXT_VALUE"'/' "$ENV_FILE" >"$TMP_FILE"
  else
    cat "$ENV_FILE" >"$TMP_FILE"
    printf '\nACADEMIC_YEAR_ROLLOVER_ENABLED=%s\n' "$NEXT_VALUE" >>"$TMP_FILE"
  fi

  mv "$TMP_FILE" "$ENV_FILE"
  trap - EXIT
  echo "Academic year rollover flag diubah: $CURRENT_VALUE -> $NEXT_VALUE"
fi
